...
import org.maxgamer.rs.events.mob.persona.PersonaDeathEvent;
import org.maxgamer.rs.events.mob.persona.PersonaStartEvent;
...
public abstract class Persona extends Mob implements YMLSerializable, InventoryHolder {
	 ...
	 * the world config.
	 */
	public static final Location PLAYER_SPAWN = Location.deserialize(Core.getServer().getConfig().getSection("spawn"), Core.getServer().getMap(), new Location(Core.getServer().getMap(), 3221, 3220, 0));
	...
	public ConfigSection serialize() {
		...
		if (loc != null) {
			this.config.set("location.x", loc.x);
			this.config.set("location.y", loc.y);
			this.config.set("location.z", loc.z);
		}
