...
public class ItemDefinition extends Definition {
	...
	public static ItemDefinition[] forName(String name) {
		...
		ItemDefinition[] protos = new ItemDefinition[ids.size()];

		for (int i = 0; i < ids.size(); i++) {
			protos[i] = ItemDefinition.getDefinition(ids.get(i));
		}
	...
	public static ItemDefinition getDefinition(Integer id) {
		ItemDefinition proto = definitions.get(id);
		if (proto == null) {
			try {
				...
				if (rs.next()) {
					...
					if (rs.getObject("type") != null) {
						org.maxgamer.rs.model.item.weapon.Equipment w = new org.maxgamer.rs.model.item.weapon.Equipment(id);
						w.reload(rs);
						proto.weapon = w;
					}
	...
	public static void init() throws Exception {
		Connection con = Core.getWorldDatabase().getConnection();
		PreparedStatement ps = con.prepareStatement("SELECT i.id, i.name FROM Item i");
		ResultSet rs = ps.executeQuery();
		while (rs.next()) {
	...
	public ItemDefinition toCharges(int n) {
		try {
			int end = this.name.lastIndexOf('(');
			String next = this.name.substring(0, end) + '(' + n + ')';

			ItemDefinition[] options = ItemDefinition.forName(next);
			for (ItemDefinition proto : options) {
	...
	public boolean isNoted() {
		...
		try{
			...
			ItemDefinition last = ItemDefinition.getDefinition(id - 1);
