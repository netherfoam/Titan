...
public class ItemProto extends Definition {
	 ...
	 * @throws RuntimeException if the item couldn't be loaded (SQL, etc)
	 */
	...
	private String[] groundOptions = new String[] { null, "Take", null, null, null };
...
/*	@Mapping
	...
	public boolean stacksWith(int id) {
		if (stackIds == null) return false;
		
		for (int stackId : this.stackIds) {
			if (stackId == id) return true;
		}
		return false;
	}
	...
	public AttackStyle getAttackStyle(int slot) {
		...
		if (o == null) return new AttackStyle(1, "Punch", Bonus.ATK_CRUSH, SkillType.ATTACK);
	...
	private void readOpcodeValues(ByteBuffer buffer) {
		...
		if (buffer.remaining() > 0) System.out.println("Buffer remaining " + buffer.remaining());
	}
