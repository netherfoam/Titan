...
public abstract class Container implements Cloneable, Iterable<ItemStack>, YMLSerializable {
	 ...
	 * 
	 * @param stack
	 *            The method of stacking this container will use.
	 */
	 ...
	 * 
	 * @param slot
	 *            The slot to get the item from, guaranteed to 0 >= slot >=
	 *            this.getSize().
	 ...
	 * 
	 * @param l
	 *            the container listener to remove
	 */
	 ...
	 * @param items
	 *            The items to transfer to the target container.
	 ...
	 * 
	 * @param c
	 *            The container to copy items from
	 ...
	 * 
	 * @param c
	 *            The container with a list of items to remove.
	 * @return True for success false for failure. If returning false, this
	 *         inventory will not be modified.
	 */
	 ...
	 * 
	 * @param iStack
	 *            The item to search for and delete all of.
	 ...
	 * 
	 * @param slot
	 *            The slot to change the item at.
	 * @param item
	 *            The item to place at the slot.
	 * @param reverse
	 *            True if you want to shuffle items up to make space. False if
	 *            you want to shuffle them down to make space.
	 * @throws IllegalArgumentException
	 *             If the item will not fit (AKA, shuffle in that direction)
	 */
	 ...
	 * 
	 * @param slotFrom
	 *            The item to insert from
	 * @param slotTo
	 *            The item to insert to.
	 */
	 ...
	 * 
	 * @param needles
	 *            The items to check it contains.
	 ...
	 * 
	 * @param c
	 *            The container to check to add items from.
	 ...
	 * 
	 * @param needle
	 *            The Item to search for. The quantity of this item is used.
	 ...
	 * 
	 * @param item
	 *            The item to search for, only the ID is used
	...
	private static boolean add(ItemStack add, ItemStack[] proto, StackType type, IntList updates, int preferredSlot) {
		...
		for (int i = (preferredSlot >= 0 ? -1 : 0); i < proto.length; i++) {
			...
			if (stack == null)
				continue; // Skip over empty spaces 'til later.
		...
		for (int i = (preferredSlot >= 0 ? -1 : 0); i < proto.length; i++) {
			...
			ItemStack stack = proto[index];
			if (stack != null)
				continue; // We've already checked filled spaces.
