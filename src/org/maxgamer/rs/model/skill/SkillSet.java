...
public class SkillSet implements YMLSerializable {
	 ...
	 *             if skilltype is null or if exp is null
	 */
	public void setExp(SkillType t, double exp) {
		...
		if (t == SkillType.ATTACK || t == SkillType.STRENGTH
				|| t == SkillType.DEFENCE || t == SkillType.CONSTITUTION
				|| t == SkillType.RANGE || t == SkillType.MAGIC
				|| t == SkillType.PRAYER) {
			int combat = 3;
			combat = (int) ((getLevel(SkillType.DEFENCE)
					+ getLevel(SkillType.CONSTITUTION) + Math
					.floor(getLevel(SkillType.PRAYER) / 2)) * 0.25) + 1;
	 ...
	 * 
	 * @param t
	 *            the {@link SkillType} to change the level of
	 * @param level
	 *            the level for the new skill, must be >= 0.
	 * @throws NullPointerException
	 *             if the {@link SkillType} is null
	 ...
	 * 
	 * @param t
	 *            the {@link SkillType} to get the level of
	 ...
	 *            the skill type
	 * @param modifier
	 *            true if you want to include modifiers, false otherwise
	 ...
	 *            the modifier to set
	 * @throws NullPointerException
	 *             if the skill is null
	 */
	public void setModifier(SkillType t, double modifier) {
		if (t == null)
			throw new NullPointerException("SkillType may not be null");
		if (getLevel(t, false) + modifier < 0)
			modifier = -getLevel(t, false);
	 ...
	 * 
	 * @param t
	 *            the {@link SkillType} to get the modifier of
	 * @return the level of the skill type
	 * @throws NullPointerException
	 *             if the given skill type is null
	 */
	 ...
	 * 
	 * @param t
	 *            The skill to modify/buff
	 * @param multiplier
	 *            the multiplier, where 1 = normal, 1.1 = 10% boost, 0.9 = 10%
	 *            decrease.
	 */
	 ...
	 * 
	 * @param type
	 *            the skill type to boost
	 * @param levels
	 *            the number of levels to restore
	 * @throws IllegalArgumentException
	 *             if the given levels to restore is < 0
	 */
	public void restore(SkillType type, double levels) {
		if (type == null)
			throw new NullPointerException("SkillType may not be null");
		if (levels < 0)
			throw new IllegalArgumentException("Levels to restore must be >= 0");
	 ...
	 * 
	 * @param map
	 *            the {@link ConfigSection} for the skill
	 */
	@Override
	...
	private int getTargetConfigValue(boolean experienceTarget) {
		...
		for (int index = 1; index < skills.size() + 1; index++) {
			...
			if (skill.isTargeting()) {
				if (experienceTarget && skill.getTargetLevel() < 0)
					continue;
				value += 1 << index;
			}
		}
