...
public class Harvestable {
	 ...
	 * @param skill the skill type for harvesting this, must not be null if
	 *        level or exp is non-zero
	 * @param level the level required to harvest this, must be non-zero if
	 *        skill is non-null
	 ...
	 *        modifiers
	 * @param minResidual the minimum number of times this object may be
	 *        harvested before replenished
	 * @param maxResidual the maximum number of times this object may be
	 *        harvested before replenished
	 * @param replaceObjId the replacement object id when repleneshing, may be
	 *        -1
	...
	public Harvestable(ItemStack reward, SkillType skill, int level, double exp, int minTime, int maxTime, int minResidual, int maxResidual, int replaceObjId, int respawnTicks, HarvestTool... tools) {
		...
		for (HarvestTool t : tools) {
			if (t == null) throw new NullPointerException("HarvestTools may not be null");
		}
	...
	public HarvestTool getTool(Persona p) {
		...
		if (tools == null || tools.length <= 0) return null; //No tool available to harvest this.
	 ...
	 * @param p the persona attempting to harvest this object
	 ...
	 * @param g the object to replenish
	 */
