...
import org.maxgamer.rs.events.mob.persona.player.PlayerMapUpdateEvent;
...
public class Game637Protocol extends GameProtocol {
	...
	public void invoke(int script, Object... params) {
		...
		if (cs2 == null) {
			try {
				...
				//There's no elegant way of handling this. We really shouldn't have to catch an IOException
				//each time.
	...
	public void sendUpdates() {
		...
		MBR visibleArea = viewport; //A MBR that overlaps with all entities that the player can see with their view distance.
		...
		RSOutgoingPacket update = new RSOutgoingPacket(-1); //This is a dummy packet. We use it later.
		out.startBitAccess();
		...
		while (pit.hasNext()) {
			...
			if (rating == -1 || rating >= MAX_LOCAL_PLAYERS || p.isHidden()) {
				...
				if (p.getUpdateMask().hasChanged() || p.getModel().hasChanged()) {
					...
					//How do we know which player we're specifying 'has no updates?'
					//In the case where there are 0 updates, the client will need to receive
					//a number of '0' bits which is equal to the number of players they
		...
		for (int i = 0; i < nearby.size() && localPlayers.size() < MAX_LOCAL_PLAYERS; i++) {
			...
			if (localPlayers.contains(p) || p.isHidden()) {
				//No need to check if p == getPlayer() because nearby has had nearby.remove(getPlayer())
				continue;
			}
		...
		if (change || System.currentTimeMillis() - lastPlayerUpdate > 1800) { //must send at least 1 update every 3 ticks, just to keep client alive.
			lastPlayerUpdate = System.currentTimeMillis();
			//Other players were updated in the update block. They go after the client's player.
		...
		//A NPC has to be within this cube to be added to the player. This is a limitation of the protocol.
		visibleArea = new Cube(new int[] { playerLoc.x - 31, playerLoc.y - 31, playerLoc.z }, new int[] { 63, 63, 0 });
		
		//Adding new NPCs
		//for (NPC n : viewport.getCenter().getMap().getEntities(visibleArea, 40, NPC.class)) {
	...
	private void applyMovementUpdate(RSOutgoingPacket out, Persona p) {
		...
		//TODO: This is probably incorrect, if it is 0, it will probably just be a 0 indicating no movement!
		out.writeBits(1, 1); //status boolean, if false, this removes the player.. Or does it?
	...
	private void appendPlayerUpdateBlock(RSOutgoingPacket out, Persona p, boolean isNew) {
		...
		if (mu.getMovement().hasTeleported()) {
			...
			//The client already knows we're teleporting, so what else does this
			//say?
			buffer.writeByteC(1);
			...
			for (Entry<Mob, ArrayList<Damage>> e : hits.entrySet()) {
				for (Damage d : e.getValue()) {
					if (d.getType() == DamageType.MISS && this.p != e.getKey() && this.p != d.getTarget()) {
						continue; //Don't send hits which are 0's and not involved with this player (waste of bandwidth)
					}
					size++;
					if (size >= 255) {
						size = 255;
		...
		if (mu.getAnimation() != null) {
			...
			//We write this four times, the best guess seems to be that Jagex was planning on using mixed animations
		...
		if (p.getModel().hasChanged() || isNew) {
			...
			buffer.write(data);
			//Log.debug("model update, " + p.getModel().hasChanged() + ", " + forceModelUpdate);
		}
		...
		//This works very similar to how you'd imagine a 3-byte smart value would work.
		if (mask > 0x80) {
			mask |= MASK_2BYTE; //if the biggest bit is set, then it means 'there is another byte coming with data'
	...
	public void chunkUpdate(Location tile) {
		...
		if (x > 127 || y > 127 || x < -128 || y < -128) {
			//We'll have an overflow, we cannot update this chunk without teleporting the player closer.
	...
	public void login() {
		...
		bldr.writeByte((byte) 0); //This is the player rights. Disabled to allow for normal command execution
		bldr.writeByte((byte) 0);
		bldr.writeByte((byte) 0);
		bldr.writeByte((byte) 0);
		bldr.writeByte((byte) 1);
		bldr.writeByte((byte) 0);
		bldr.writeShort((short) (getPlayer().getSpawnIndex() + 1)); //Player id's are 1-2047
		bldr.writeByte((byte) 1);
	...
	public void sendMap() {
		...
		//TODO: If this Viewport is constructed while the player is near the edge of the map,
		//then it will overlap the edge of the map and thus not fit in the WorldMap. This will
		//cause errors.
		...
		if (m instanceof StandardMap) {
			...
			out.writeByte(getPlayer().getViewDistance().getId()); //Viewport depth
			...
			for (int regionX = (chunkX - mapHash) >> 3; regionX <= (chunkX + mapHash) >> 3; regionX++) {
				for (int regionY = (chunkY - mapHash) >> 3; regionY <= (chunkY + mapHash) >> 3; regionY++) {
					...
					try {
						...
						//Instead of raising an exception, the client loads the region anyway. However,
				...
				for (int chunkX = (cx - mapHash); chunkX <= (cx + mapHash); chunkX++) {
					for (int chunkY = (cy - mapHash); chunkY <= (cy + mapHash); chunkY++) {
						...
						if (c == null) {
							...
							out.writeBits(10, c.getCacheX()); //I have no idea why this is 10 and not 11.
							out.writeBits(11, c.getCacheY()); //Maybe x can only go up to 8191 for dynamic maps? 
		...
		//Dynamic objects are objects which aren't in the cache and must be added if visible
		...
		//Static objects are objects which are in the cache and must be removed if hidden
	...
	private void loginData(RSOutgoingPacket out) {
		...
		short playerId = (short) (getPlayer().getSpawnIndex() + 1); //Network ID is +1.
	...
	public void sendLocalMessage(Persona sender, String s, int effects) {
		...
		byte[] data = new byte[256]; //Has to be 256, in case huffman actually causes it to get bigger.
	...
	public void close() {
		localPlayers.clear();
		localNpcs.clear();
		//Most importantly, remove the viewport from the map. It has a reference to the player.
	...
	public void receivePrivateMessage(String username, int rights, String message) {
		long id = (long) (1 + ((Math.random() * Long.MAX_VALUE) + (Math.random() * Long.MIN_VALUE)));//What is this for?
		byte[] bytes = new byte[256];
